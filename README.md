...
disables is never grantable. `--policy-file` adds an optional
      ...
      template:
        spec:
          ...
          - name: package-runtime
            ...
            - --policy-file=/etc/function-wasm/policy.cedar
            volumeMounts:
...
permits it: with a `--policy-file` set, permit every capability you mean to
allow. `function validate --policy-file …` reports the same verdicts offline
