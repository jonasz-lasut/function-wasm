...
| `--metrics-label-input-name` | `METRICS_LABEL_INPUT_NAME` | `false` | add an `input` label (the Input's `metadata.name`, empty when unset) to `run_duration_seconds`, `http_requests_total`, `requests_total` and `run_instructions`. The cardinality is bounded by the set of Compositions the operator's own cluster uses; the risk (hundreds of Inputs) is the operator's to decide |
