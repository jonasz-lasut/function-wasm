...
// TestRunFunctionEgress pins how a module's required egress is admitted before
// it runs: the manifest is the request, refused with no --sandbox-policy-file
// to grant it, otherwise compiled and the module runs. The operator host
// allowlist is Cedar's grantEgress (see validate_test).
func TestRunFunctionEgress(t *testing.T) {
	wasm := testwasm.Fixed(t, guestResponse(), testwasm.Options{})
	host := publicRegistry(t)
	egressRef := pushWithManifest(t, host+"/egress:v1", wasm,
		`{"abi":1,"requires":{"egress":{"http":[{"host":"api.example.com","methods":["GET"]},{"hostPattern":"*.eu.internal.example.com","methods":["POST"],"pathPrefix":"/v1/"}]}}}`)
	// The egress mechanism. The operator host allowlist is Cedar's (grantEgress,
	// at admission); the permissive policy admits any required host, so this
	// exercises the grant-compile and run path, not host capping (that is the
	// OperatorPolicy* cases in the admission tests).
	open, err := egress.New()
	if err != nil {
		t.Fatal(err)
	}
	...
	cases := map[string]struct {
		...
		"NoPolicy": {
			reason: "With no --sandbox-policy-file to grant egress, a module's required egress is a fatal result, after the manifest is read and before it runs.",
			args: args{egress: open, req: &fnv1.RunFunctionRequest{
				Meta:  &fnv1.RequestMeta{Tag: "hello"},
				Input: input(t, map[string]any{"type": "OCI", "oci": map[string]any{"ref": egressRef}}),
			}},
			want: want{rsp: fatal("module oci " + egressRef + " requires egress (requires.egress.http), but the runtime has no --sandbox-policy-file, which is required to grant egress (grantEgress)")},
		},
		"Granted": {
			reason: "A policy grant lets the module run; a module that never calls the import is unaffected. Host capping is Cedar's grantEgress, exercised in the admission tests, not here.",
...
// wasmfn.HTTPClient through the whole host: the manifest's required rules
// admit or refuse, the host performs the request and re-enters the guest's
// wasmfn_alloc for the answer, and the audit line carries the module's
// identity.
func TestRunFunctionEgressGuest(t *testing.T) {
	...
	// The guest with its egress request as a manifest, and the same guest
	// without one - the default sandbox, where the import refuses.
	host := publicRegistry(t)
	grantedRef := pushWithManifest(t, host+"/httpguest:v1", wasm,
		`{"abi":1,"requires":{"egress":{"http":[{"host":"127.0.0.1","methods":["GET","POST"],"pathPrefix":"/api/"}]}}}`)
	plainRef := push(t, host+"/plainguest:v1", wasm)
	...
	audit := func(ref string) string {
		return "Module HTTP request tag=hello module=oci " + ref + " digest=" + strings.SplitN(ref, "@", 2)[1] + " method="
	}
	...
	oci := func(ref string) map[string]any {
		return map[string]any{"module": map[string]any{"type": "OCI", "oci": map[string]any{"ref": ref}}}
	}
	...
	cases := map[string]struct {
		...
		"Get": {
			...
			ref:    grantedRef,
			config: map[string]any{"url": srv.URL + "/api/items"},
			want:   normal("200 GET /api/items guest=httpguest body="),
			log:    audit(grantedRef) + "GET outcome=ok",
		},
		"PostBody": {
			reason: "A request body travels to the server.",
			ref:    grantedRef,
			config: map[string]any{"url": srv.URL + "/api/items", "method": "POST", "body": "hi"},
			want:   normal("200 POST /api/items guest=httpguest body=hi"),
			log:    audit(grantedRef) + "POST outcome=ok",
		},
		"MethodRefused": {
			reason: "A method outside the manifest's rules is a transport error in the guest, which it reports as a fatal result.",
			ref:    grantedRef,
			config: map[string]any{"url": srv.URL + "/api/items", "method": "DELETE"},
			want:   fatal(`Delete "` + srv.URL + `/api/items": wasmfn: sandbox.egress: no rule for host "127.0.0.1" admits DELETE /api/items`),
			log:    audit(grantedRef) + "DELETE outcome=refused",
		},
		"PathRefused": {
			reason: "A path outside the rules' prefix is refused.",
			ref:    grantedRef,
			config: map[string]any{"url": srv.URL + "/admin"},
			want:   fatal(`Get "` + srv.URL + `/admin": wasmfn: sandbox.egress: no rule for host "127.0.0.1" admits GET /admin`),
			log:    audit(grantedRef) + "GET outcome=refused",
		},
		"NoRequest": {
			reason: "A module whose manifest requires no egress gets none: the import refuses and the guest sees the reason.",
			ref:    plainRef,
			config: map[string]any{"url": srv.URL + "/api/items"},
			want:   fatal(`Get "` + srv.URL + `/api/items": wasmfn: sandbox.egress: HTTP egress is not granted to this module: its manifest requires no egress (requires.egress.http)`),
			log:    audit(plainRef) + "GET outcome=refused",
		},
	}
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
