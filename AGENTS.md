...
│       sandbox.Validate (shape); Ceiling.Grant(input.sandbox) → private      │
...
│       runOptions(input.limits, engine.Config()) → engine.RunOptions, or a    │
│       fatal naming the limit and the ceiling flag; withSandbox adds the grant│
...
```
cmd/function/main.go        kong CLI → engine.New, openCaches (afero stores under /tmp/function-wasm-cache), module.NewResolver, sandbox.NewCeiling (the --enable-sandbox-* flags, checked at startup), egress.LoadPolicy/New (only with --enable-sandbox-egress), engine.NewCache → health NOT_SERVING → function.Serve(&Function{}) with warm-up (below) flipping it to SERVING
cmd/function/fn.go          Function.RunFunction: the seven steps above; load (cache Get + fetch log, shared with warm-up); registryAuth for OCI step credentials; withSandbox (sandbox.Grant → engine.RunOptions); egress grant → per-run HTTP client on RunOptions
...
internal/engine             wasmtime wrapper — the only production importer of github.com/bytecodealliance/wasmtime-go/vNN (internal/testwasm uses its Wat2Wasm)
  engine.go                   Engine (config incl. MaxConcurrentRuns → run slots, on-demand epoch ticker + runs_in_flight gauge, linker with WASI + wasmfn.log + wasmfn.http; native unwind info off), Compile + checkABI (exports, both imports' types), Module leases; RunOptions (limits + Preopens/PrivateTmp/Env + HTTP), Preopen
...
internal/module             ModuleSource → Ref{Digest, Description, Fetch}
  ...
  policy.go                   ValidatePolicy (shape; credentialsAllowList requires repositoryAllowList) and admit(): an XR-chosen OCI/HTTP source requires repositoryAllowList; its normalized location (registry/repository, scheme://host/path — dot segments refused by Validate) must match a prefix, credentials only from credentialsAllowList and only within those repositories; static sources bypass
...
├── internal/{egress,engine,module,sandbox,testwasm}
...
3. Wire it: a source option → `internal/module.Validate` / the resolver; something the XR may choose → `FromComposite`/`policy.go` (never read `policy`, `limits` or `sandbox` from the composite); a run budget → `runOptions` in `cmd/function/fn.go` + `engine.RunOptions`; a sandbox grant → `internal/sandbox` (`Validate` for the shape, `Ceiling`/`Options` for its `--enable-sandbox-*` flag, `Grant` for the check) → `engine.RunOptions` + `configureSandbox` for the mechanics
...
- `docs/one-pager-admission-tooling.md` — `function validate` (the runtime's own admission over a Composition, offline), the pure-Go wasm reader `internal/wasmbin` behind `guestfn inspect`/`build`/`push` and a pre-compile ABI reject, `layerDigests` in pushed artifacts (draft)
- `docs/one-pager-module-manifest.md` — the `wasmfn.manifest` custom section (required capabilities, `config` schema, min runtime) mirrored to OCI annotations, written by `guestfn build`, checked against the grant at load — narrowing only (draft)
- `docs/one-pager-local-loop.md` — `function run`, `guestfn dev`, request capture with redaction and replay, the per-run summary line, per-run guest stderr with the panic header, readable traps (draft)
