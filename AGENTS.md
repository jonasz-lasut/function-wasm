...
│       grant and the flag; sandbox.egress → egress.Grant (rules) then the    │
│       operator's grantEgress, or a fatal naming the rule / the flag;         │
...
- **`sandbox.egress is refused: the runtime was started without --enable-sandbox-egress`**: the Composition grants egress but the operator did not enable it — set the flag (the host allowlist and CIDR rules are in the Cedar `--sandbox-policy-file`).
- **`sandbox.filesystem.privateTmp is refused: the operator policy (--sandbox-policy-file) does not permit it for this request`** (also `sandbox.env …` and `sandbox.egress.http[i] METHOD to host "x" is refused: the operator policy …`): the operator's Cedar grant policy is default-deny and permits no matching rule for this caller (by `principal.namespace`/`xrKind`). Add a `permit` for the capability, or drop the grant. Once a `--sandbox-policy-file` is set, an `--enable-sandbox-*` flag is necessary but no longer sufficient - the policy still has to permit it.
