...
internal/engine             wasmtime wrapper — the only production importer of github.com/bytecodealliance/wasmtime-go/vNN (internal/testwasm uses its Wat2Wasm)
  ...
  run.go                      Run(ctx, m, req, log, RunOptions): run slot under ctx (not timed/counted if never obtained), private /tmp then store per call, deadline/limiter (Config capped by RunOptions), fuel (SetFuel/GetFuel, observeFuel → run_instructions histogram, OutOfFuel → ErrFuel), configureSandbox, ABI calls, guestError/trapText
