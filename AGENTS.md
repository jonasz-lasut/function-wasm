...
│       /tmp (shape); the operator policy enables usePrivateTmp / setEnv, or   │
│       a fatal; sandbox.egress → operator grantEgress then egress.Grant, or   │
│       a fatal naming the rule; limits → engine.RunOptions, or a fatal        │
│       naming the limit and the ceiling flag; module.Validate +               │
│       ValidatePolicy (shape)                                                 │
...
internal/authz              Cedar PDP (cedar-go, the only importer; stays CGo-free): the always-on built-in fences over a Composition's own allowlists - RepositoryFence (pullModule) and CredentialFence (spendCredential), static policies with the allowlist in request context (injection-safe) over a boundary-correct Repository hierarchy (a location's ancestors are its path-boundary prefixes, both slash forms), so a prefix never admits a sibling namespace or an adjacent host - and OperatorPolicy, the operator's --sandbox-policy-file grant document: PermitsPrivateTmp/PermitsEnv/PermitsEgress (default-deny grant gating, a Request principal carrying namespace/xrKind/composition, a HostPattern DNS-suffix hierarchy for egress), RequiresSignature (per-repository, pre-crypto) + HasSignatureRules, and CompileIPRules (Action::"dialAddress" forbid/permit -> a netip.Prefix decision table for egress, fail-closed on any unrecognized shape). Refusal strings stay in the callers (module, admission). docs/one-pager-policy-engine.md
...
internal/module             ModuleSource → Ref{Digest, Description, Fetch}
  ...
  oci.go / http.go / path.go  one source each; oci resolveOCI builds fetch/manifest/verify closures; keys on the manifest digest, fetches the manifest only inside Fetch and stores the layer by its digest; Ref.Manifest reads the artifact's manifest layer (ManifestLayer; verified, <= manifest.MaxSize; path/http: none); WasmLayer (the layer rule, shared with guestfn inspect; the manifest layer never counts as the only layer), IsTarLayer, ExtractWasm (a tar layer must hold ScratchModulePath = /fn.wasm exactly - nothing is guessed)
  ...
  policy.go                   ValidatePolicy (shape; credentialsAllowList requires repositoryAllowList), ValidateFrom (what a from source needs without the XR: shape + the repositoryAllowList fence; function validate without --xr) and admit(): an XR-chosen OCI/HTTP source requires repositoryAllowList; its normalized location (registry/repository, scheme://host/path — dot segments refused by Validate) must lie within a prefix (the repository check is authz.RepositoryFence, Cedar, boundary-correct), credentials only from credentialsAllowList (exact set membership) and only within those repositories; static sources bypass
...
internal/sandbox            Validate (shape of the Input's sandbox: http rules host XOR hostPattern - bare host names and one-wildcard patterns via egress.ValidHost/ValidHostPattern - methods from the enum, normalized pathPrefix; env[] name/value/valueFrom shape, envFrom[] credential/prefix shape, duplicate detection), RequestsEgress, RequestsEnv;
                            Options/NewCeiling (the operator's flags, checked once: a private /tmp probed under os.TempDir()), Ceiling.Grant(sandbox) → Grant{PrivateTmp} or an error naming the grant and the flag; a nil Ceiling allows nothing. No mounts: host directories are deliberately not mountable into a module;
                            Materialize(sandbox, Sources) → resolved env map: resolves env[] literals and valueFrom credential refs, envFrom[] bulk imports, withholds the pull credential, checks NUL/duplicates/key validity
internal/egress             HTTP egress through the host (sandbox.egress): New(...Option) → Egress (fixed default budgets + DefaultBlockedCIDRs; options: WithBlockedCIDRs/WithAllowedCIDRs carry the operator's Cedar Action::"dialAddress" rules compiled by authz, WithRateLimit the --egress-rate-limit-* flags, WithBudgets a test/future knob since operators cannot tune the budgets) → Grant(rules) (compiles the Composition's own rules; there is no host ceiling here - the operator host allowlist is Cedar's grantEgress at admission) → Client(log, digest) per run: rate limit check (process-wide per module digest), admit (scheme, host/pattern, method, normalized path prefix), dialer that resolves and judges every address then dials the checked one, redirects re-checked, budgets, metric + audit line; Request/Response (internal/egress/wire) are the ABI's JSON payloads. No cedar-go here, and blockedBy stays allocation-free (explicit block > allow > DefaultBlockedCIDRs). There is no --sandbox-egress-policy file: hosts and CIDRs are Cedar, budgets are fixed, the rate limit is a flag
...
type Input struct {
    ...
    Sandbox *Sandbox             `json:"sandbox,omitempty"` // Filesystem{PrivateTmp}, Env[]EnvVar{Name,Value|ValueFrom}, EnvFrom[]EnvFromSource{Credential,Prefix}, Egress{HTTP} - each enabled by the operator's Cedar --sandbox-policy-file; host directories are not mountable
    Config  *runtime.RawExtension `json:"config,omitempty"` // opaque; the guest reads it via wasmfn.GetConfig
}
```

`module.type` is always the Composition's choice; with `from` the composite resource chooses the instance (an `{ref, credentials}` object for OCI, `{url, digest}` for HTTP, a string for Path), and `policy` — read from the Input only — says which repositories (prefixes) it may name and which step credentials it may spend (only within those repositories; a credentials list without a repository list is refused). `limits` narrow one run below `--module-timeout` / `--module-memory-limit`; more than a ceiling is a fatal result naming both. `sandbox` is validated for shape (`internal/sandbox.Validate`) and enabled by the operator's Cedar `--sandbox-policy-file` (the sole enabler, at admission): `filesystem.privateTmp` gives the run an empty writable `/tmp` (`usePrivateTmp`) — the only directory a module can be given, host directories are deliberately not mountable — `env` sets exactly those variables (`setEnv`); a grant no policy permits is a fatal result naming the grant, and `egress` is HTTP through the host (`grantEgress`, also the host allowlist, and the CIDR rules `dialAddress`, in the Cedar `--sandbox-policy-file`, `internal/egress`: a grant the operator policy does not permit, or any grant on a runtime with no `--sandbox-policy-file`, is a fatal result before the module runs; the sandbox one-pager). The design of the whole shape: `docs/one-pager-module-source-schema.md`.
...
- **Sandbox types before behaviour**: the `sandbox` subtree shipped as validated types with a "not implemented yet" refusal so Compositions and tooling could settle on the schema; each capability then landed with behaviour. The private `/tmp`, environment and HTTP egress (the host allowlist and CIDR rules in `--sandbox-policy-file`) are all in.
...
- `docs/one-pager-sandbox.md` — the sandbox grants: a private `/tmp` (host mounts deliberately not offered), environment and HTTP egress through the host, each enabled by the operator's Cedar `--sandbox-policy-file` (implemented)
...
- `docs/one-pager-request-secrets.md` — secrets and files delivered from the request into env and the private `/tmp`, never from the host; the `env` shape decision (implemented: env/envFrom; request-delivered files not yet)
...
- **`sandbox.filesystem.privateTmp is refused: the runtime has no --sandbox-policy-file, which is required to grant sandbox capabilities`** (also `sandbox.env …`, and `sandbox.egress is refused: … to grant egress (grantEgress)`): the runtime has no operator policy, so no sandbox capability is grantable — mount a Cedar `--sandbox-policy-file` with a matching `usePrivateTmp`/`setEnv`/`grantEgress` permit (a `DeploymentRuntimeConfig` sets args and the ConfigMap) or drop the grant.
...
- **`sandbox.egress is refused: the runtime has no --sandbox-policy-file, which is required to grant egress (grantEgress)`**: the Composition grants egress but the operator has no policy — mount a Cedar `--sandbox-policy-file` with a `grantEgress` permit (which is also the host allowlist; the CIDR rules are `dialAddress`).
- **`sandbox.filesystem.privateTmp is refused: the operator policy (--sandbox-policy-file) does not permit it for this request`** (also `sandbox.env …` and `sandbox.egress.http[i] METHOD to host "x" is refused: the operator policy …`): the operator's Cedar grant policy is default-deny and permits no matching rule for this caller (by `principal.namespace`/`xrKind`). Add a `permit` for the capability, or drop the grant.
...
- **A guest's request fails with `wasmfn: sandbox.egress: HTTP egress is not granted to this module`**: the module calls `wasmfn.HTTPClient()` but its Composition has no `sandbox.egress.http` rule (or the runtime has no `--sandbox-policy-file` granting egress); the import always exists, the grant decides.
