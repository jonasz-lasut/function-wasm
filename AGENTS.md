...
| the CLI | `cmd/guestfn` (root module) | `guestfn init` scaffolds a guest project, `guestfn build` compiles it (and checks its ABI with the runtime's engine), `guestfn inspect` shows what the runtime sees in a module or an artifact, `guestfn push` publishes it (refusing a module the runtime would refuse) — CGo, like the runtime |
...
cmd/function/fn.go          Function.RunFunction: the seven steps above; ceilings() → admission.Ceilings; load (cache Get + fetch log, shared with warm-up); registryAuth for OCI step credentials; egress grant → per-run HTTP client on RunOptions
...
internal/module             ModuleSource → Ref{Digest, Description, Fetch}
  ...
  oci.go / http.go / path.go  one source each; oci keys on the manifest digest, fetches the manifest only inside Fetch and stores the layer by its digest; WasmLayer (the layer rule, shared with guestfn inspect), IsTarLayer, ExtractWasm (a tar layer must hold ScratchModulePath = /fn.wasm exactly — nothing is guessed)
...
cmd/guestfn                 CLI (CGo — it links internal/engine for the runtime's own verdicts): main.go (init --lang go|tinygo|rust, build with toolchain detection + engine.Inspect verdict, push refusing a non-ABI module and writing the wasm config with layerDigests through a partial.CompressedImageCore), inspect.go (file → engine.Inspect; reference → manifest, layers, annotations, module.WasmLayer, --pull), internal/scaffold
...
├── cmd/guestfn/            the CLI (package main): main.go, inspect.go + internal/scaffold/{scaffold.go,vendorproto.go,templates/<lang>/,testdata/<lang>/}
├── internal/{admission,egress,engine,module,sandbox,testwasm}
