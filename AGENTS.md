...
```
cmd/function/main.go        kong CLI: CLI{Debug, Serve (default:"withargs"), Validate, Precompile}; CeilingFlags embedded in all three (module-dir, max-module-size, module-timeout, module-memory-limit, cosign-key, enable-sandbox-*, sandbox-egress-policy, enable-fuel, module-instruction-limit, registry-mirror, oci-layout-dir) with ceilings() (sandbox.NewCeiling checked at startup, egress.LoadPolicy/New only with --enable-sandbox-egress -> admission.Ceilings) and resolver(blobs) (mirrors normalized, layout opened); engineConfig() wires Fuel and InstructionLimit into engine.Config; ServeCmd.Run -> engine.New, openCaches(fuel) (afero stores under /tmp/function-wasm-cache, Version(fuel) namespaces the compiled store), engine.NewCache -> health NOT_SERVING -> function.Serve(&Function{}) with warm-up (below) flipping it to SERVING; main maps validate's and precompile's exitError to the exit code
...
cmd/function/precompile.go  PrecompileCmd: the warm path as a standalone command that exits non-zero on failure; constructs a minimal Function (engine, cache, resolver, manifests) and runs fn.precompile (same resolve -> verify -> load -> release as warmOne, at most --max-concurrent-compiles at once, but failures are counted and a non-zero count yields exitError{1})
cmd/function/fn.go          Function.RunFunction: the eight steps above; ceilings() → admission.Ceilings; codec.ConsumeRequest (stashed wire bytes, consumed before credential stripping may invalidate them); load (cache Get + fetch log, shared with warm-up); checkManifest/manifestFor (memory → manifests store → Ref.Manifest; parsed once per digest); registryAuth for OCI step credentials; egress grant → per-run HTTP client on RunOptions; stepSlots.Acquire for limits.concurrency before runModule (raw path via engine.RunRaw when no credential stripping, else engine.Run); codec.StashResponse when the guest set meta so the codec's Marshal skips re-encoding
...
internal/engine             wasmtime wrapper — the only production importer of github.com/bytecodealliance/wasmtime-go/vNN (internal/testwasm uses its Wat2Wasm)
  engine.go                   Engine (config incl. MaxConcurrentRuns → fairScheduler, MaxTotalRunMemory → memPool, Fuel + InstructionLimit → fuel metering, on-demand epoch ticker + runs_in_flight gauge, linker with WASI + wasmfn.log + wasmfn.http; native unwind info off; SetConsumeFuel when Fuel is on), Compile + checkABI (exports, both imports' types), Module leases; RunOptions (limits + Instructions + PrivateTmp/Env + HTTP + Key + InputName); Config.WithDefaults; Version(fuel) appends -fuel when fuel is on — artifacts are not interchangeable; ErrFuel sentinel
  ...
  run.go                      Run (marshal req → runWith → unmarshal rsp), RunRaw (runWith with caller's bytes → rsp + raw bytes), runWith (the core: run slot under ctx (not timed/counted if never obtained), memory reservation from memPool (when --max-total-run-memory is set), private /tmp then store per call, deadline/limiter (Config capped by RunOptions), fuel (SetFuel/GetFuel, observeFuel → run_instructions histogram, OutOfFuel → ErrFuel), configureSandbox, ABI calls, guestError/trapText)
...
internal/codec              raw-bytes gRPC codec: registered as "proto" via encoding.RegisterCodec in init(); Unmarshal decodes normally and stashes a clone of the wire bytes for *fnv1.RunFunctionRequest (weak.Pointer + runtime.AddCleanup side table); Marshal returns stashed bytes for *fnv1.RunFunctionResponse (when the guest set meta), else marshals normally; falls through to standard proto for non-RunFunction messages; StashRequest/ConsumeRequest, StashResponse/consumeResponse
internal/module             ModuleSource → Ref{Digest, Description, Fetch}
  module.go                   Resolver, Options (Blobs *cache.Store, Mirrors map[string]string, LayoutDir string), Validate (type + exactly one of its object or from, no foreign object; digest-pinned oci refs, required http digest), Resolve switches on type, verified() (blob store + digest check), timed() (fetch metric); NewResolver normalizes mirror keys and opens the OCI layout
  layout.go                   OCI layout helpers: layoutManifest (read manifest by digest from the layout), layoutBlob (read blob by hash), openLayout (validate + open the directory)
  oci.go / http.go / path.go  one source each; oci resolveOCI builds fetch/manifest/verify closures that check the OCI layout (layoutManifest, layoutBlob) before the network and apply the registry mirror (mirrorOf: rewrites fetchRef, uses keychain auth) when configured; keys on the manifest digest, fetches the manifest only inside Fetch and stores the layer by its digest; Ref.Manifest reads the artifact's manifest layer (ManifestLayer; verified, <= manifest.MaxSize; path/http: none); WasmLayer (the layer rule, shared with guestfn inspect; the manifest layer never counts as the only layer), IsTarLayer, ExtractWasm (a tar layer must hold ScratchModulePath = /fn.wasm exactly - nothing is guessed)
  signature.go                Verifier: cosign key-based signature check; Verify (remote, per-process cache) and VerifyFromLayout (OCI layout: finds the .sig manifest in index.json by ref-name annotation, reads signature layers from blobs, falls through to network on miss)
...
internal/egress             HTTP egress through the host (sandbox.egress): Policy (the --sandbox-egress-policy file, defaults, DefaultBlockedCIDRs, optional RateLimit) → New → Egress (the ceiling, with optional per-module-digest token bucket rate limiter) → Grant(rules) (intersection, refuses rules outside the ceiling) → Client(log, digest) per run: rate limit check (process-wide per module digest), admit (scheme, host/pattern, method, normalized path prefix), dialer that resolves and judges every address then dials the checked one, redirects re-checked, budgets, metric + audit line; Request/Response are the ABI's JSON payloads
...
├── internal/{admission,codec,egress,engine,manifest,module,sandbox,testwasm}
...
- `docs/one-pager-governance-perf.md` — fuel-based `limits.instructions` (implemented), per-module egress rate limits (implemented), per-Input concurrency, registry mirror + OCI-layout source + `function precompile`, raw-bytes gRPC codec — phases over the resource-governance one-pager (draft, phases 1-2 implemented)
...
- **`module … failed: wasmfn_run failed: module exceeded its instruction budget (N instructions)`**: the module exhausted its fuel budget (`limits.instructions` or `--module-instruction-limit`); raise the limit or optimise the module. The `run_instructions` histogram shows the distribution.
...
- **`function precompile` exits 1**: at least one module failed to compile - the log line carries the entry and the reason. Unlike `--warm-modules`, precompile surfaces the failure; use it in an init container or Job over the shared cache volume.
- **`cannot open OCI layout at /modules`**: `--oci-layout-dir` points at a directory without `oci-layout` and `index.json` (the OCI image-layout structure); prepare it with `crane pull --format=oci`, `oras copy --to-oci-layout`, or `cosign save`.
- **A module fetches from the mirror but auth fails**: `--registry-mirror` uses the runtime's Docker config (`DOCKER_CONFIG`) for the mirror, not step credentials; configure the mirror's credentials in the runtime pod's Docker config.
