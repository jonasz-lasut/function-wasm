...
| one run, HTTP egress (`sandbox.egress`) | per-run budgets (fixed defaults): request count, response bytes, redirects, per-request timeout; a request is also cut at the run's deadline | fixed defaults (not operator-configurable) | 16, 4 MiB, 5, 10 s | the request is answered with an error the guest sees; the run goes on |
